...
class CommentModel extends BaseModel {
    ...
    function get_comment_info($id, $type) {
        ...
        $info['favorable_count'] = empty($favorable['count']) ? '0' : $favorable['count'];      //好评数量
        $info['medium_count'] = empty($medium['count']) ? '0' : $medium['count'];               //中评数量
        $info['bad_count'] = empty($bad['count']) ? '0' : $bad['count'];                        //差评数量
        if ($info['count'] > 0) {
            $info['favorable'] = 0;
            if ($favorable) {
