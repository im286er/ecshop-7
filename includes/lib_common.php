 ...
 * @return  $combined
 */
function index_get_new_comments($num = 6)
{
	$sql = 'SELECT c.*,g.goods_id,g.goods_name,g.goods_thumb FROM '.$GLOBALS['ecs']->table('comment').' AS c '.
			' LEFT JOIN '.$GLOBALS['ecs']->table('goods') .' AS g'.
			' ON c.id_value = g.goods_id WHERE comment_type = 0 AND status = 1 ORDER BY c.comment_id DESC LIMIT '.$num;
	...
	foreach($res as $idx=>$row)
	{
		...
		$arr[$idx]['short_name'] = $GLOBALS['_CFG']['goods_name_length'] > 0 ? sub_str($row['goods_name'], $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
		...
		$arr[$idx]['email']      = $row['email'];
		$arr[$idx]['comment_rank'] = $row['comment_rank'];
		$arr[$idx]['content']    = $row['content'];
		$arr[$idx]['add_time']   = date($GLOBALS['_CFG']['date_format'], $row['add_time']);
	}
	
	return $arr;
}

function new_comment($goods_id,$num = 1)
{
	$sql = 'SELECT * FROM '.$GLOBALS['ecs']->table('comment')." WHERE id_value = '$goods_id' AND comment_type=0 AND status = 1 AND parent_id = 0 ORDER BY comment_id DESC LIMIT ".$num;
