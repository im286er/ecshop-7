...
function insert_ads($arr) {
    ...
    if (!empty($arr['num']) && $arr['num'] != 1) {
        $sql = 'SELECT a.ad_id, a.position_id, a.media_type, a.ad_link, a.ad_code, a.ad_name, a.start_time, a.end_time, p.ad_width, ' .
    ...
    foreach ($res AS $row) {
        ...
        switch ($row['media_type']) {
            case 0: // 图片广告
                ...
                border='0' /></a>";
                $ads_array[$row['ad_id']]['ad_img'] = "<a href='" . url('default/affiche/index', array('ad_id' => $row['ad_id'], 'uri' => urlencode($row["ad_link"]))) . "'
                target='_blank'><img src='$src' width='" . $row['ad_width'] . "' height='$row[ad_height]'
                border='0' /></a>";
                $ads_array[$row['ad_id']]['ad_name'] = $row['ad_name'];
                $ads_array[$row['ad_id']]['start_time'] = local_date("Y-m-d G:i:s", $row['start_time']);
                $ads_array[$row['ad_id']]['end_time'] = local_date("Y-m-d G:i:s", $row['end_time']);
                $ads_array[$row['ad_id']]['time'] = ceil(($row['end_time']-$time)/(60*60*24));
                break;
            case 1: // Flash
                $src = (strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'], 'https://') === false) ?
                        __URL__ . "/$row[ad_code]" : $row['ad_code'];
    ...
    ECTouch::view()->assign('ads_array', $ads_array);
