...
//-- 快速查看ajax处理
/*------------------------------------------------------ */
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'quickView')
{
	include('includes/cls_json.php');

    $json   = new JSON;
    $res    = array('error' => '', 'content' => '');
...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
		...
		$smarty->assign('same_brand',         get_same_brand($goods['brand_id'],5,$goods_id)); 
		
		$smarty->assign('same_category',         get_same_category($goods['cat_id'],5,$goods_id)); 
		
		$smarty->assign('same_category_best',         get_same_category_best($goods['cat_id'],5,$goods_id)); 
