...
if (!$smarty->is_cached($dwt_name.'.dwt', $cache_id))
{
	...
	{
		$smarty->assign('parent_id_show',         $cat_id);
	}
	
	/*范备*/
	$sql = 'SELECT cat_desc FROM '.$ecs->table('category').' WHERE cat_id ='.$cat_id;
	$cat_desc = $db->getOne($sql);
	$smarty->assign('cat_desc', $cat_desc);//商品描述
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name,b.brand_name,g.brand_id, g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		$sql = 'SELECT give_integral FROM '.$GLOBALS['ecs']->table('goods')." WHERE goods_id = '$row[goods_id]'";
		
		$give_integral = $GLOBALS['db']->getOne($sql);
		
		if($give_integral == -1)
		{
			if($row['promote_price'] > 0)
			{
				$arr[$row['goods_id']]['give_integral']      = $promote_price;
			}
			else
			{
				$arr[$row['goods_id']]['give_integral']      = $row['shop_price'];
			}
		}
		else
		{
			$arr[$row['goods_id']]['give_integral']      = $give_integral;
		}
	
		$arr[$row['goods_id']]['brand_url'] =  build_uri('brand', array('cid' => 0, 'bid' => $row['brand_id']), $row['brand_name']);
		...
		$arr[$row['goods_id']]['end_date']     = $row['promote_end_date'];
