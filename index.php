...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
	...
	$smarty->assign('like_goods',       get_recommend_goods('like'));     // 猜你喜欢
	...
	$smarty->assign('new_comments',    index_get_new_comments());   // 最新评论
    $smarty->assign('group_buy_goods', index_get_group_buy());      // 团购商品
	
    $smarty->assign('auction_list',    index_get_auction());        // 拍卖活动
    $smarty->assign('shop_notice',     $_CFG['shop_notice']);       // 商店公告
	
	$smarty->assign('seckill_goods', get_seckill_goods()); // 秒杀商品
 ...
 $smarty->assign('script_name',      'index'); // 分类树
    /* 页面中的动态内容 */
...
function index_get_group_buy()
{
    ...
    if ($limit > 0)
    {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
			...
			$len = strpos($price,'.');
			$row['market_price']=substr($price,'0',$len);
