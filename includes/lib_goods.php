...
/**同品牌**/
function get_same_brand($brand,$limit = 5,$goods_id)
{
	$sql = 'SELECT g.goods_id, g.goods_name,g.brand_id, g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g  ' .
			 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
				" WHERE brand_id = '$brand' AND g.goods_id != '$goods_id' limit ".$limit;
...
/**同分类**/
function get_same_category($cat,$limit = 5,$goods_id)
{
	$sql = 'SELECT g.goods_id, g.goods_name,g.brand_id, g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g  ' .
			 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
				" WHERE cat_id = '$cat' AND g.goods_id != '$goods_id' limit ".$limit;
...
/**积分热卖商品**/
function get_exchange_hot($goods_id,$limit = 5)
{
	$sql = 'SELECT eg.exchange_integral,g.goods_id, g.goods_name,g.brand_id, g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
			...
			' ON eg.goods_id = g.goods_id '.
			 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
				" WHERE eg.is_hot = 1 AND g.goods_id != '$goods_id' limit ".$limit;
...
/**同分类**/
function get_same_category_best($cat,$limit = 5,$goods_id)
{
	$sql = 'SELECT g.goods_id, g.goods_name,g.brand_id, g.goods_name_style, g.comments_number,g.sales_volume,g.market_price, g.is_new, g.is_best, g.is_hot, g.shop_price AS org_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, g.promote_price, g.goods_type, " .
                'g.promote_start_date, g.promote_end_date, g.goods_brief, g.goods_thumb , g.goods_img ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g  ' .
			 'LEFT JOIN ' . $GLOBALS['ecs']->table('member_price') . ' AS mp ' .
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' " .
				" WHERE cat_id = '$cat' AND g.goods_id != '$goods_id' AND g.is_best = 1 limit ".$limit;
...
/**
 * 获得秒杀抢购商品 
 *authour ecmoban模板堂 shao start
 * @access  public
 * @return  array
 */
function get_seckill_goods($cats = '')
{
    $time = gmtime();
   //print_r(local_date('Y-m-d H:i:s', $time));
   $order_type = $GLOBALS['_CFG']['recommend_order'];

    /* 取得促销lbi的数量限制 */
    $num = get_library_number("recommend_seckill");
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price,g.seckill_number, g.seckill_start_date, g.seckill_end_date,  g.promote_price, g.seckill_price,' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name, " .
                "g.is_best, g.is_new, g.is_hot, g.is_promote, g.is_seckill, RAND() AS rnd " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
            "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ".
            'WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' .
            " AND g.is_seckill = 1 AND seckill_end_date > '$time' ";
...
//        {
         // $seckill_price = bargain_price($row['seckill_price'], $row['seckill_start_date'], $row['seckill_end_date']);
          $goods[$idx]['seckill_price'] = $row['seckill_price'] > 0 ? price_format($row['seckill_price']) : '';
//        }
//        else
//        {
//            $goods[$idx]['seckill_price'] = '';
//        }
        ...
        $time = gmtime();
       	if($time < $row['seckill_start_date'])
        {
           $goods[$idx]['gmt_end_time']  = local_date('m d, Y H:i:s',$row['seckill_start_date']);
        }
        elseif($time > $row['seckill_start_date'] && $time < $row['seckill_end_date'])
		{
			$goods[$idx]['gmt_end_time']  = local_date('m d, Y H:i:s',$row['seckill_end_date']);
		}
		else
        {
        ...
        $goods[$idx]['seckill_number']  = $row['seckill_number'];
        $goods[$idx]['seckill_start_date']  = $row['seckill_start_date'];
        $goods[$idx]['seckill_end_date']    =$row['seckill_end_date'];
        $goods[$idx]['now'] = $time;
       	//$goods[$idx]['seckill_start_date']  = local_date('Y-m-d H:i:s', $row['seckill_start_date']);
        //$goods[$idx]['seckill_end_date']    =local_date('Y-m-d H:i:s', $row['seckill_end_date']);
    }

    return $goods;
}
/*ecmoban 模板堂 shao end*/
...
{
    if (!in_array($type, array('best', 'new', 'hot', 'like')))
    {
        return array();
    }
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.market_price, g.shop_price, g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_like, g.is_promote, b.brand_name,g.sort_order ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
               ' LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
               ' WHERE g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND (g.is_best = 1 OR g.is_new =1 OR g.is_hot = 1 OR g.is_like)'.
            ...
            if (!empty($goods_res))
            {
                foreach($goods_res as $data)
                {
					...
					if ($data['is_like'] == 1)
                    {
                        $goods_data['like'][] = array('goods_id' => $data['goods_id'], 'sort_order' => $data['sort_order']);
                    }
                    if ($data['brand_name'] != '')
                    {
        ...
        $type2lib = array('best'=>'recommend_best', 'new'=>'recommend_new', 'hot'=>'recommend_hot' ,'like'=>'recommend_like');
        if (empty($type_array))
        {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.comments_number, g.sales_volume, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "LEFT JOIN " . $GLOBALS['ecs']->table('member_price') . " AS mp ".
                "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
        $type_merge = array_merge($type_array['new'], $type_array['best'], $type_array['hot'] ,$type_array['like']);
        ...
        foreach ($result AS $idx => $row)
        {
			...
			if (in_array($row['goods_id'], $type_array['like']))
            {
                $type_goods['like'][] = $goods[$idx];
            }
        }
    }
...
function get_promote_goods($cats = '')
{
    ...
    foreach ($result AS $idx => $row)
    {
		...
		$goods[$idx]['promote_start_date']           = $row['promote_start_date'];
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit,c.cat_name,c.cat_id, b.brand_id, b.brand_logo, g.comments_number, g.sales_volume,b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
    ...
    if ($row !== false)
    {
		...
		$row['cat_url']   = build_uri('category', array('cid' => $row['cat_id']), $row['cat_name']);
        return $row;
    }
    else
    {
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    if ($from == 'web')
    {
		$goods['get_top']=get_top10($cat_id);
		$goods['id'] = $cat_id;
