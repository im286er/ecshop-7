...
elseif($_REQUEST['act']  == 'query_install' || $_REQUEST['act'] == 'update_website')
{
	...
	foreach($_POST['zocms'] as $key => $val)
	{
...
elseif($_REQUEST['act'] == 'init')
{
	...
	$oauth_arr = array(0=>'show_oauth_qq',1=>'show_oauth_weibo',2=>'show_oauth_alipay',3=>'show_oauth_taobao');
	foreach($oauth_arr as $oauth)
	{
		$sql = 'SELECT 1 FROM '.$ecs->table('shop_config').' WHERE parent_id=3 and code=\''.$oauth.'\'';
		$row = $db->getRow($sql);
		
		if(!$row)
		{
			$sql = "INSERT INTO ".$ecs->table('shop_config')." ( `id`, `parent_id`, `code`, `type`, `store_range`, `store_dir`, `value`, `sort_order` ) VALUES ('', '3', '".$oauth."', 'select', '1,0', '', '1', '0');";
		$db->query($sql);
		}
	}
...
function getWebsiteList()
{
	...
	{
		if($file != '.' && $file != '..' && $file != 'zocms.php' && $file != 'config' && $file != 'tb_callback.php' && $file != 'tb_index.php' && substr($file , strlen($file)-4) == '.php' && substr($file , 0 , 3) != 'cls')
		{
