...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy')
{
    ...
    if ($is_add)
    {
        ...
        $goods = array(
            ...
            'give_integral' => -1,
            'rank_integral' => -1,
            //商品抢购 ecmoban模板堂 shao
            'seckill_price' => 0,
			'seckill_number'=> 0,
            'seckill_start_date' => local_date('Y-m-d H:i:s'),
            'seckill_end_date'   => local_date('Y-m-d H:i:s', local_strtotime('+1 month'))
        );
    ...
    else
    {
        ...
        if (empty($goods) === true)
        {
            ...
            $goods = array(
				...
				'seckill_price' => 0,
				'seckill_number'=> 0,
				'seckill_start_date' => local_date("Y-m-d H:i:s",$goods['seckill_start_date']),
				'seckill_end_date'   => local_date("Y-m-d H:i:s",$goods['seckill_end_date'])
            );
        }
		...
		if (!empty($goods['packing_list']))
        {
            //$goods['goods_brief'] = trim_right($goods['goods_brief']);
            $goods['packing_list'] = $goods['packing_list'];
        }
		if (!empty($goods['after_sales']))
        {
            //$goods['goods_brief'] = trim_right($goods['goods_brief']);
            $goods['after_sales'] = $goods['after_sales'];
        }
        ...
        //如果不是抢购 处理抢购日期 ecmoban模板堂 shao start
        
        if (isset($goods['is_seckill']) && $goods['is_seckill'] == '0')
        {
            unset($goods['seckill_start_date']);
            unset($goods['seckill_end_date']);
        }
        else
        {
            $goods['seckill_start_date'] =  local_date("Y-m-d H:i:s",$goods['seckill_start_date']);
            $goods['seckill_end_date'] = local_date("Y-m-d H:i:s",$goods['seckill_end_date']);
        }
    //如果不是抢购 ecmoban模板堂 shao end
        /* 如果是复制商品，处理 */
    ...
    /* 创建 html editor */
    create_html_kindeditor('goods_desc', $goods['goods_desc']);
	
	create_html_kindeditor2('seller_info', $goods['seller_info']);
	
	create_html_kindeditor3('brand_info', $goods['brand_info']);
...
//商品抢购添加代码(最下边三行) ecmoban模板堂 shao 
    /* 处理商品数据 */
    //用户自定义时间转换成GMT时间
    ...
    $seckill_price = !empty($_POST['seckill_price']) ? floatval($_POST['seckill_price'] ) : 0;
    $is_seckill = empty($_POST['is_seckill']) ? 0 : 1;
    $seckill_start_date = !empty($_POST['seckill_start_date']) ? local_strtotime($_POST['seckill_start_date']) : 0;
    $seckill_end_date = !empty($_POST['seckill_end_date']) ? local_strtotime($_POST['seckill_end_date']) : 0;
	$seckill_number = isset($_POST['seckill_number']) ? $_POST['seckill_number'] : 0;
    
    /* 入库 */
    //商品秒杀抢购 ecmoban模板堂 shao (insert语句插入) 
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, packing_list, after_sales, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, give_integral, is_best, is_new, is_hot,  " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, seller_info, brand_info, add_time, last_update, goods_type, rank_integral, suppliers_id,seckill_price,seckill_start_date,seckill_end_date,is_seckill, seckill_number)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[packing_list]', '$_POST[after_sales]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '$_POST[seller_info]', '$_POST[brand_info]','" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id','$seckill_price','$seckill_start_date','$seckill_end_date','$is_seckill','$seckill_number')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, seller_info, brand_info, add_time, last_update, goods_type, extension_code, rank_integral,seckill_price,seckill_start_date,seckill_end_date,is_seckill,seckill_number)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[packing_list]', '$_POST[after_sales]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '$_POST[seller_info]', '$_POST[brand_info]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$seckill_price','$seckill_start_date','$seckill_end_date','$is_seckill','$seckill_number')";
        }
    }
    else
    {
    ...
    //商品秒杀抢购 ecmoban模板堂 shao 更新语句插入
        $sql = "UPDATE " . $ecs->table('goods') . " SET " .
                ...
                "promote_end_date = '$promote_end_date', ".
                 ...
                 "seckill_start_date = '$seckill_start_date', " .
                "seckill_end_date = '$seckill_end_date', ";
...
elseif ($_REQUEST['act'] == 'drop_goods')
{
	...
	/* 后台删除产品时自动删除产品详情图片 ecmoban by fanbo */
